...
analytics.internal.GoogleAnalyticsService.prototype.getTracker =
    function(trackingId) {
  // Each tracker instance has its own ServiceChannel - this is necessary in
  // order for each tracker to have its own EventTarget instance.
  var serviceChannel = new analytics.internal.ServiceChannel(
      this.settings_,
      analytics.internal.GoogleAnalyticsService.ChannelPipelineFactory,
      analytics.internal.DummyChannel.getInstance());
  var eventTarget = serviceChannel.getEventTarget();

  var tracker =
      new analytics.internal.ServiceTracker(serviceChannel, eventTarget);
